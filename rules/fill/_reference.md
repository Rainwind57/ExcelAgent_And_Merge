...
> 本文件为详细参考，**不自动注入解析 prompt**。注入 prompt 的硬约束见 `_global.md`。
> 数据来源：`resources/` 下各 `.xlsx` 的 Row1(中文表头) / Row2(英文字段名:类型) / Row3(约束) / Row4(默认值)。
> 校验规则（机器强约束）见 `rules/validate/`。
...
**关键陷阱**：
- Row1 表头常含 `\n` 换行尾注，匹配列名时必须取 `\n` 之前的主名。
- Row2 字段名有点分嵌套路径（如 `effect.data.3006.conv_id`），不是真实列名，是结构化字段键，需点分翻译。
...
2. `effect.data.N.*` → 提取 N，找 `_clean_header == N` 的列
...
item.ItemBase (主表，item_id 主键)
  ├─ item_type 决定子表：1资源 / 2礼包→Chest / 3药品→Potion / 4ao
  └─ item_id ←── 子表共享主键
...
新增时必须提供值，不可留空。必填列详见 `rules/validate/` 的 `required` 字段。

### 4.2 `export_type: ONLY_SERVER` — 仅服务端导出

`entity_prefab.Base.交互id` 标记 `export_type: ONLY_SERVER`，表示该列只服务端使用。

### 4.3 `alias:X` — 列别名

`Potion` 表 Row3：`药品数值范围(小)` → `alias:c`、`药品数值范围(高)` → `alias:d`。
`道具描述` 用 `${a}` / `${c}` / `${d}` 引用（如 `回复${a}血量`）。

### 4.4 `ref_col:True` — 引用其他列

`Potion.道具描述` 标记 `ref_col:True`，值含 `${变量}` 占位符，引用同表其他列。
...
| `填表说明(不导出)` | 人类备注，程序跳过 |
| `区域说明[不导出]` | 区域备注 |
| `名字（不导表）` | 名称冗余列（主名在 ItemBase.name） |
| `描述(不导表)` | 描述冗余 |
| `部位（不导表）` | 部位冗余 |
...
4. **枚举值**：枚举列取值是否在允许集合内？
5. **跨表级联**：新增 NPC → 同步建 interaction + conv + option + spawn 行等。
6. **外键存在性**：引用的 FK 目标行是否已存在？
7. **默认值**：未指定列是否可用 Row4 默认值？

### 7.2 修改（modify）校验

1. **行定位**：row_key 目标行是否真实存在？
2. **字段匹配**：列名能否命中表头？
3. **类型保持**：新值类型是否与列类型一致？
4. **枚举约束**：新枚举值是否合法？
5. **FK 影响面**：修改主键 ID → 引用该 ID 的子表行需同步更新？

### 7.3 删除（delete）校验

1. **级联清理**：删主表 → 同步删关联子表行。
2. **反向引用检查**：被删行是否被其他表引用？
...
|---|---|---|
| 列名带 `\n` 尾注 | 匹配失败 | 取 `\n` 前主名匹配 |
| `effect.key` 直送 matcher | 点分键非真实列 | 先点分翻译 |
| list 写成 JSON | `'["a","b"]'` | 用逗号分隔串 |
| 枚举写中文标签 | `quality='凡品'` | 先枚举映射 `quality=1` |
| 主键冲突 | 新增 ID 已存在 | 取 max+1，不硬编码 |
| 漏子表级联 | 只写主表 | 按 §2 外键图补全 |
| 修改未存在行 | row_key 查不到 | 先确认行存在 |
