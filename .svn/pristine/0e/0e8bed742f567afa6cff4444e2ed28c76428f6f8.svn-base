...
|---|---|---|---|---|
| 定位功能 | 命中正确 table+sheet+操作类型的比例 | 0.4722 | 0.6389 | +0.1667 |
| 覆盖度 | expected 行操作中真正被产出的比例（含级联多表） | 0.4167 | 0.5833 | +0.1666 |
| 精准程度 | 被定位到的行里，字段值完全正确的比例 | 0.4972 | 0.5898 | +0.0926 |
| 严格通过率 | 整条样例 expected_answer 100%命中且无多余写入 | 0.1667 | 0.1667 | +0.0000 |
| 响应ok率 | Agent 自报告执行成功比例 | 0.5000 | 0.3333 | -0.1667 |
| 平均多余写入 | 未被 expected 认领的行改动数（越低越好） | 1.3333 | 1.3333 | -0.0000 |
| 平均耗时(ms) | 单条指令端到端耗时（含二次确认续传） | 86397.9 | 115858.9 | -29461.0 |
| P50耗时(ms) | | 46813.6 | 114687.1 | |
| P95耗时(ms) | | 262520.9 | 218530.2 | |
| 需二次确认比例 | 触发级联删除等确认流程的比例 | 0.0000 | 0.0000 | |
| 总耗时(s) | 本轮全部样例累计耗时 | 518.4 | 695.2 | |

## 二、按操作类型细分（skill=on）

| 操作类型 | n | 定位率 | 覆盖率 | 字段精准度 | (对比 off) |
|---|---|---|---|---|---|
| add | 14 | 0.7857 | 0.7857 | 0.6894 | off: locate=0.5714 cov=0.5714 acc=0.7292 |
...
| 多余写入(误写行数) | 4 | 5 |
| 耗时(ms) | 132780 | 195904 |
| 错误 | 失败：coerce_value - 列[3001: 战斗ID]类型为 int，值''无法转为整数且无枚举映射，已阻止写入 | 失败：coerce_value - 列[选项3]类型为 int，值''无法转为整数且无枚举映射，已阻止写入 |
...
| 覆盖度(行操作产出率) | 0.67 | 1.00 |
| 精准程度(字段值正确率) | 0.58 | 0.64 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 2 | 1 |
| 耗时(ms) | 46814 | 114687 |
| 错误 | 失败 | 失败：pk_conflict - ID [100081000] 已被占用 |
...
|---|---|---|
| 响应ok | False | False |
| 定位功能(table/sheet命中率) | 0.33 | 0.33 |
| 覆盖度(行操作产出率) | 0.00 | 0.00 |
| 精准程度(字段值正确率) | 0.67 | 0.67 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 1 | 1 |
| 耗时(ms) | 262521 | 218530 |
...
| 2 | interaction.xlsx.Interaction | add | ✅ 完全匹配 (1.00) | ✅ 完全匹配 (1.00) |
| 3 | spawn_world_entity.xlsx.SpawnWorldEntity | add | ❌ 未产生 (0.00) | ❌ 未产生 (0.00) |
...
|---|---|---|---|---|
| 1 | interaction.xlsx.InteractionConv | modify | ❌ 未产生 (0.00) | ❌ 未产生 (0.00) |
| 2 | interaction.xlsx.InteractionConvOption | add | ❌ 未产生 (0.00) | ✅ 完全匹配 (1.00) |
...
| 7 | 修改conv_id为1的对话内容为'你好，旅行者，欢迎来到我们的村庄。' | 0.00 | 0.00 | 0.00 | 失败：write - 写后验证不符 |
| 6 | 新增一个擂台NPC叫'擂台挑战者'，model_id为1012，放在space_ | 0.00 | 0.67 | 0.33 | 失败：pk_conflict - ID [1065601] 已被占用 |
| 8 | 给conv_id为4的帮派青龙堂总管对话新增一个选项，选项内容为'我想了解帮派贡 | 0.67 | 0.50 | 0.67 | - |
| 1 | 新增一个NPC叫铁匠老张，model_id为1015，放在space_id 10 | 0.83 | 0.73 | 0.83 | 失败：coerce_value - 列[选项3]类型为 int，值''无法转为整数且无枚举映射，已阻止写入 |
| 4 | 新增一个传送NPC叫'传送使者'，model_id为1016，放在space_i | 1.00 | 0.64 | 1.00 | 失败：pk_conflict - ID [100081000] 已被占用 |
...
- skill 挂载后定位功能变化: +0.1667（提升）
- skill 挂载后覆盖度变化: +0.1666（提升）
- skill 挂载后精准程度变化: +0.0926（提升）
- skill 挂载后严格通过率变化: +0.0000
- skill 挂载后耗时变化: -29461.0ms（变慢，正数=on比off快，因为定义为 off-on）
- 综合判定: skill 增强 有效
