...
|---|---|---|---|---|
| 定位功能 | 命中正确 table+sheet+操作类型的比例 | 0.0800 | 0.1467 | +0.0667 |
| 覆盖度 | expected 行操作中真正被产出的比例（含级联多表） | 0.0000 | 0.0000 | +0.0000 |
| 精准程度 | 被定位到的行里，字段值完全正确的比例 | 0.0000 | 0.0000 | +0.0000 |
| 严格通过率 | 整条样例 expected_answer 100%命中且无多余写入 | 0.0000 | 0.0000 | +0.0000 |
| 响应ok率 | Agent 自报告执行成功比例 | 0.2000 | 0.0000 | -0.2000 |
| 平均多余写入 | 未被 expected 认领的行改动数（越低越好） | 2.0000 | 1.8000 | +0.2000 |
| 平均耗时(ms) | 单条指令端到端耗时（含二次确认续传） | 72907.9 | 112515.8 | -39607.9 |
| P50耗时(ms) | | 91113.1 | 104396.7 | |
| P95耗时(ms) | | 101609.2 | 200559.9 | |
| 需二次确认比例 | 触发级联删除等确认流程的比例 | 0.0000 | 0.0000 | |
| 总耗时(s) | 本轮全部样例累计耗时 | 364.5 | 562.6 | |

## 二、按操作类型细分（skill=on）

| 操作类型 | n | 定位率 | 覆盖率 | 字段精准度 | (对比 off) |
|---|---|---|---|---|---|
| add | 16 | 0.1875 | 0.1875 | 0.4127 | off: locate=0.1250 cov=0.1250 acc=0.3333 |
...
| 4 | combat/spell.xlsx.common_spell | add | 🟠 定位到但字段不符 (0.00) | 🟠 定位到但字段不符 (0.00) |
...
| 多余写入(误写行数) | 1 | 0 |
| 耗时(ms) | 101609 | 75445 |
| 错误 | 失败：locate_row - 未找到 77777001(mode=exact) | 失败：parse - 解析失败（parse_failed） |
...
| 多余写入(误写行数) | 7 | 6 |
| 耗时(ms) | 101156 | 200560 |
| 错误 | 失败 | 失败：pk_conflict - ID [11] 已被占用 |

expected_answer 逐条判定（skill=off → skill=on）：

| # | table.sheet | op | off | on |
|---|---|---|---|---|
| 1 | residence/residence_building.xlsx.BuildingType | add | ❌ 未产生 (0.00) | 🟡 部分匹配 (0.57) |
...
| 多余写入(误写行数) | 0 | 0 |
| 耗时(ms) | 68361 | 66190 |
| 错误 | 失败：locate_row - 未找到 28501(mode=contains) | 失败：locate_row - 未找到 28501(mode=exact) |
...
| 108 | 给蜀山（门派 1）加一个新神通"，神通编号 8099，描述"驭三柄飞剑斩敌"，图 | 0.00 | 0.00 | 0.40 | 失败：pk_conflict - ID [5] 已被占用 |
| 109 | 加一条日常打怪任务"清剿赤炎虎"，任务号 777001，归到任务组 510（组名 | 0.00 | 0.00 | 0.00 | 失败：parse - 解析失败（parse_failed） |
| 110 | 洞府新增一种炼丹房。先把建筑类型 11 建好，叫"炼丹房"，实体类型 Resid | 0.00 | 0.00 | 0.33 | 失败：pk_conflict - ID [11] 已被占用 |
...
- skill 挂载后定位功能变化: +0.0667（提升）
- skill 挂载后覆盖度变化: +0.0000（持平）
- skill 挂载后精准程度变化: +0.0000（持平）
- skill 挂载后严格通过率变化: +0.0000
- skill 挂载后耗时变化: -39607.9ms（变慢，正数=on比off快，因为定义为 off-on）
- 综合判定: skill 增强 效果不明显
