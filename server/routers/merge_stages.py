...
def _build_group(
    ...
    # table_stem 用于 compare_sheet 查 value_constraints 列类型做公式重算结果类型校验
    table_stem = group_name.rsplit("/", 1)[-1] if group_name else None
    if workers > 1:
        def _cmp_sheet(sn):
            return sn, compare_sheet(file_sheets, base_name, sn, file_formulas, detect_missing=detect_missing, file_comments=file_comments, merge_base_file=merge_base_file, commit_authors=commit_authors, version_meta=version_meta, sparse=True, table_stem=table_stem)
        with ThreadPoolExecutor(max_workers=workers) as ex:
            for sn, merged in ex.map(_cmp_sheet, sheet_list):
                merged_by_sheet[sn] = merged
    else:
        for sheet_name in sheet_list:
            merged_by_sheet[sheet_name] = compare_sheet(file_sheets, base_name, sheet_name, file_formulas, detect_missing=detect_missing, file_comments=file_comments, merge_base_file=merge_base_file, commit_authors=commit_authors, version_meta=version_meta, sparse=True, table_stem=table_stem)
