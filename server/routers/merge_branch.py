...
def _git_commit_apply(repo_root: Path, commit_files: List[Path], commit_name: str) -> dict:
    ...
    提交范围：仓库内所有已跟踪文件的修改（含 _append_audit 更新的审计日志）。
    merge/svn/demo_svn 下的合并产物由 SVN 管理且被 .gitignore 排除，不进 git。
    失败不抛异常，只回传 error（SVN 已落盘，git 失败不影响结果）。
    """
    ...
    try:
        ...
        if r.returncode != 0:
            stdout = r.stdout.decode("utf-8", "replace") if isinstance(r.stdout, bytes) else str(r.stdout)
            err = r.stderr.decode("utf-8", "replace").strip() if isinstance(r.stderr, bytes) else str(r.stderr).strip()
