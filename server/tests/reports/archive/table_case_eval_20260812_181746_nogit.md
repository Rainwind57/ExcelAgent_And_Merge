...
| 平均多余写入 | 未被 expected 认领的行改动数（越低越好） | 2.0000 | 1.0000 | +1.0000 |
| 平均耗时(ms) | 单条指令端到端耗时（含二次确认续传） | 233489.7 | 232715.9 | +773.8 |
| P50耗时(ms) | | 233489.7 | 232715.9 | |
| P95耗时(ms) | | 233489.7 | 232715.9 | |
| 需二次确认比例 | 触发级联删除等确认流程的比例 | 0.0000 | 0.0000 | |
| 总耗时(s) | 本轮全部样例累计耗时 | 233.5 | 232.7 | |
...
| 多余写入(误写行数) | 2 | 1 |
| 耗时(ms) | 233490 | 232716 |
| 错误 | 失败：match_field - 未找到列[实体名字] | 失败：match_field - 未找到列[最大生成数量] |
...
- skill 挂载后耗时变化: +773.8ms（变快，正数=on比off快，因为定义为 off-on）
- 综合判定: skill 增强 效果不明显
