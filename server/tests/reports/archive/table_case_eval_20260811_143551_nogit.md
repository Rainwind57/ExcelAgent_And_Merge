...
|---|---|---|---|---|
| 定位功能 | 命中正确 table+sheet+操作类型的比例 | 0.4512 | 0.2928 | -0.1584 |
| 覆盖度 | expected 行操作中真正被产出的比例（含级联多表） | 0.3857 | 0.1286 | -0.2571 |
| 精准程度 | 被定位到的行里，字段值完全正确的比例 | 0.2229 | 0.1548 | -0.0681 |
| 严格通过率 | 整条样例 expected_answer 100%命中且无多余写入 | 0.0000 | 0.0000 | +0.0000 |
| 响应ok率 | Agent 自报告执行成功比例 | 0.1667 | 0.1667 | +0.0000 |
| 平均多余写入 | 未被 expected 认领的行改动数（越低越好） | 4.6667 | 7.3333 | -2.6666 |
| 平均耗时(ms) | 单条指令端到端耗时（含二次确认续传） | 137844.1 | 176450.8 | -38606.7 |
| P50耗时(ms) | | 118194.9 | 164868.6 | |
| P95耗时(ms) | | 279262.1 | 260715.1 | |
| 需二次确认比例 | 触发级联删除等确认流程的比例 | 0.0000 | 0.0000 | |
| 总耗时(s) | 本轮全部样例累计耗时 | 827.1 | 1058.7 | |

## 二、按操作类型细分（skill=on）

| 操作类型 | n | 定位率 | 覆盖率 | 字段精准度 | (对比 off) |
|---|---|---|---|---|---|
| add | 23 | 0.4348 | 0.3478 | 0.4109 | off: locate=0.5217 cov=0.4783 acc=0.5299 |
| modify | 2 | 0.0000 | 0.0000 | 0.0000 | off: locate=0.5000 cov=0.5000 acc=0.0000 |
...
| 覆盖度(行操作产出率) | 0.71 | 0.57 |
| 精准程度(字段值正确率) | 0.53 | 0.42 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 6 | 3 |
| 耗时(ms) | 279262 | 248473 |
| 错误 | 失败：coerce_value - 列[选项2]类型为 int，值''无法转为整数且无枚举映射，已阻止写入 | 失败：locate_row - 未找到 灵兽饲养员老李(mode=exact) |
...
| 4 | interaction.xlsx.InteractionConvOption | add | 🟠 定位到但字段不符 (0.00) | 🟠 定位到但字段不符 (0.00) |
| 5 | interaction.xlsx.InteractionConvOption | add | ❌ 未产生 (0.00) | ❌ 未产生 (0.00) |
| 6 | interaction.xlsx.InteractionConv | add | ✅ 完全匹配 (1.00) | ❌ 未产生 (0.00) |
...
|---|---|---|
| 响应ok | False | False |
| 定位功能(table/sheet命中率) | 0.75 | 0.25 |
| 覆盖度(行操作产出率) | 0.50 | 0.00 |
| 精准程度(字段值正确率) | 0.39 | 0.18 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 4 | 4 |
| 耗时(ms) | 194634 | 163485 |
| 错误 | 失败：pk_conflict - ID [7019] 已被占用 | 失败：match_field - 未找到列[fabao_type] |
...
| 3 | combat/spell.xlsx.common_spell | add | ✅ 完全匹配 (1.00) | ❌ 未产生 (0.00) |
| 4 | combat/spell.xlsx.spell_data | add | 🟡 部分匹配 (0.50) | ❌ 未产生 (0.00) |
...
| 覆盖度(行操作产出率) | 0.60 | 0.20 |
| 精准程度(字段值正确率) | 0.41 | 0.33 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 0 | 2 |
| 耗时(ms) | 96825 | 260715 |
| 错误 | 失败：pk_conflict - ID [5] 已被占用 | 失败：coerce_value - 列[心法1被动id]类型为 int，值''无法转为整数且无枚举映射，已阻止写入 |
...
| 2 | school/school_ability.xlsx.SchoolAbilityLevel | add | 🟡 部分匹配 (0.17) | ❌ 未产生 (0.00) |
...
| 3 | combat/combat.xlsx.combat_data | add | ❌ 未产生 (0.00) | 🟡 部分匹配 (0.12) |
...
| 多余写入(误写行数) | 4 | 6 |
| 耗时(ms) | 118195 | 131697 |
| 错误 | 失败：pk_conflict - ID [11] 已被占用 | 失败：pk_conflict - ID [11] 已被占用 |
...
|---|---|---|
| 响应ok | True | False |
| 定位功能(table/sheet命中率) | 0.50 | 0.00 |
| 覆盖度(行操作产出率) | 0.50 | 0.00 |
| 精准程度(字段值正确率) | 0.00 | 0.00 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 0 | 0 |
| 耗时(ms) | 52800 | 89466 |
| 错误 | - | 失败：locate_row - 未找到 28501(mode=exact) |
...
| 2 | item/item.xlsx.Equipment | modify | 🟠 定位到但字段不符 (0.00) | ❌ 未产生 (0.00) |

## 四、skill=on 表现最差的样例（覆盖度+精准度最低 Top10）

| cid | input | 覆盖度 | 精准度 | 定位率 | 错误 |
|---|---|---|---|---|---|
| 109 | 加一条日常打怪任务"清剿赤炎虎"，任务号 777001，归到任务组 510（组名 | 0.00 | 0.00 | 0.25 | - |
...
| 107 | 加一件新法宝"玄火鉴"，法宝编号 28599，品质 3，图标 Icon_faba | 0.00 | 0.18 | 0.25 | 失败：match_field - 未找到列[fabao_type] |
| 108 | 给蜀山（门派 1）加一个新神通"，神通编号 8099，描述"驭三柄飞剑斩敌"，图 | 0.20 | 0.33 | 0.40 | 失败：coerce_value - 列[心法1被动id]类型为 int，值''无法转为整数且无枚举映射，已阻止写入 |
| 106 | 新增一个新 NPC 叫"灵兽饲养员老李"，模型用 1015，摆在野外战场 100 | 0.57 | 0.42 | 0.86 | 失败：locate_row - 未找到 灵兽饲养员老李(mode=exact) |
...
- skill 挂载后定位功能变化: -0.1584（下降）
- skill 挂载后覆盖度变化: -0.2571（下降）
- skill 挂载后精准程度变化: -0.0681（下降）
- skill 挂载后严格通过率变化: +0.0000
- skill 挂载后耗时变化: -38606.7ms（变慢，正数=on比off快，因为定义为 off-on）
- 综合判定: skill 增强 效果不明显
