...
|---|---|---|---|---|
| 定位功能 | 命中正确 table+sheet+操作类型的比例 | 0.3262 | 0.3817 | +0.0555 |
| 覆盖度 | expected 行操作中真正被产出的比例（含级联多表） | 0.2369 | 0.2369 | +0.0000 |
| 精准程度 | 被定位到的行里，字段值完全正确的比例 | 0.2464 | 0.3593 | +0.1129 |
| 严格通过率 | 整条样例 expected_answer 100%命中且无多余写入 | 0.0000 | 0.0000 | +0.0000 |
| 响应ok率 | Agent 自报告执行成功比例 | 0.0000 | 0.0000 | +0.0000 |
| 平均多余写入 | 未被 expected 认领的行改动数（越低越好） | 3.5000 | 5.8333 | -2.3333 |
| 平均耗时(ms) | 单条指令端到端耗时（含二次确认续传） | 114484.1 | 159638.9 | -45154.8 |
| P50耗时(ms) | | 110687.8 | 174249.2 | |
| P95耗时(ms) | | 168412.8 | 240687.3 | |
| 需二次确认比例 | 触发级联删除等确认流程的比例 | 0.0000 | 0.0000 | |
| 总耗时(s) | 本轮全部样例累计耗时 | 686.9 | 957.8 | |

## 二、按操作类型细分（skill=on）

| 操作类型 | n | 定位率 | 覆盖率 | 字段精准度 | (对比 off) |
|---|---|---|---|---|---|
| add | 23 | 0.5217 | 0.4348 | 0.7095 | off: locate=0.4783 cov=0.3913 acc=0.5134 |
...
| 覆盖度(行操作产出率) | 0.57 | 0.57 |
| 精准程度(字段值正确率) | 0.42 | 0.79 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 2 | 2 |
| 耗时(ms) | 110688 | 193933 |
| 错误 | 失败 | 失败：write - 写后验证不符 |
...
| 3 | interaction.xlsx.InteractionConv | add | 🟠 定位到但字段不符 (0.00) | ✅ 完全匹配 (1.00) |
| 4 | interaction.xlsx.InteractionConvOption | add | 🟠 定位到但字段不符 (0.00) | 🟡 部分匹配 (0.50) |
| 5 | interaction.xlsx.InteractionConvOption | add | ❌ 未产生 (0.00) | ❌ 未产生 (0.00) |
| 6 | interaction.xlsx.InteractionConv | add | ❌ 未产生 (0.00) | ❌ 未产生 (0.00) |
...
|---|---|---|
| 响应ok | False | False |
| 定位功能(table/sheet命中率) | 0.25 | 0.50 |
| 覆盖度(行操作产出率) | 0.25 | 0.25 |
| 精准程度(字段值正确率) | 0.43 | 0.46 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 3 | 4 |
| 耗时(ms) | 154265 | 110957 |
| 错误 | 失败：match_field - 未找到列[item_id] | 失败：pk_conflict - ID [7019] 已被占用 |
...
| 3 | combat/spell.xlsx.common_spell | add | ❌ 未产生 (0.00) | ✅ 完全匹配 (1.00) |
...
|---|---|---|
| 响应ok | False | False |
| 定位功能(table/sheet命中率) | 0.60 | 0.60 |
| 覆盖度(行操作产出率) | 0.60 | 0.60 |
| 精准程度(字段值正确率) | 0.63 | 0.63 |
| 严格通过 | False | False |
| 多余写入(误写行数) | 1 | 1 |
| 耗时(ms) | 82545 | 148200 |
...
| 2 | school/school_ability.xlsx.SchoolAbilityLevel | add | 🟡 部分匹配 (0.83) | 🟡 部分匹配 (0.83) |
...
| 多余写入(误写行数) | 14 | 27 |
| 耗时(ms) | 168413 | 240687 |
| 错误 | 失败：locate_row - 未找到 10999(mode=contains) | 失败：locate_row - 未找到 77777001(mode=exact) |
...
| 3 | combat/combat.xlsx.combat_data | add | 🟡 部分匹配 (0.62) | ❌ 未产生 (0.00) |
...
| 2 | residence/residence_building.xlsx.ResidenceBuilding | add | ❌ 未产生 (0.00) | 🟡 部分匹配 (0.60) |
...
| 110 | 洞府新增一种炼丹房。先把建筑类型 11 建好，叫"炼丹房"，实体类型 Resid | 0.00 | 0.27 | 0.33 | 失败：pk_conflict - ID [22199] 已被占用 |
| 107 | 加一件新法宝"玄火鉴"，法宝编号 28599，品质 3，图标 Icon_faba | 0.25 | 0.46 | 0.50 | 失败：pk_conflict - ID [7019] 已被占用 |
| 108 | 给蜀山（门派 1）加一个新神通"，神通编号 8099，描述"驭三柄飞剑斩敌"，图 | 0.60 | 0.63 | 0.60 | 失败：pk_conflict - ID [5] 已被占用 |
| 106 | 新增一个新 NPC 叫"灵兽饲养员老李"，模型用 1015，摆在野外战场 100 | 0.57 | 0.79 | 0.86 | 失败：write - 写后验证不符 |
...
- skill 挂载后定位功能变化: +0.0555（提升）
- skill 挂载后覆盖度变化: +0.0000（持平）
- skill 挂载后精准程度变化: +0.1129（提升）
- skill 挂载后严格通过率变化: +0.0000
- skill 挂载后耗时变化: -45154.8ms（变慢，正数=on比off快，因为定义为 off-on）
- 综合判定: skill 增强 有效
