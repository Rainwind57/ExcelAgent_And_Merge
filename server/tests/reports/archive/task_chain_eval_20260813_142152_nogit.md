...
| 平均耗时(ms) | 单链端到端 | 66691.6 |
| P50/P95(ms) | | 66691.6 / 66691.6 |
| 总耗时(s) | | 66.7 |
...
| field_error | 2 | 3 | 字段值写错/枚举未解析/类型不符 → 补 column_aliases/enum_mappings |
| ref_broken | 2 | 3 | 占位符替换错误或 consumer 字段名错 → 修 OperationOrchestrator._capture_produced 列名派生 |
...
- 定位 1.00 | 覆盖 1.00 | 精准 0.00 | 引用一致 0.00 (0/2) | producer产出 2/2
- 多余写入 3 (异表 0) | 耗时 66692ms
...
|---|---|---|---|---|---|
| 3 | False | 0.00 | 1.00 | 0.00 | 新增一封全服邮件'周年庆奖励'，mail_id 8001，邮件类型1，标 |

## 五、内环优化建议

- 字段错误 2 处：补 column_aliases / enum_mappings / value_constraints，强化枚举值预解析与类型校验。
- 引用断裂 2 处 + producer 未产出 0 处：这是 task_chain 核心瓶颈。核查 OperationOrchestrator._capture_produced 主键列名派生（首列 col==1 优先）与 _resolve_placeholders 占位符替换覆盖；确保 add 结果 result_rows 回传主键新值，produces 标签与占位符名对齐。
