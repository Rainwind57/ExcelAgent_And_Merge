...
| 定位功能 | 命中正确 table+sheet+操作类型 | 0.6667 |
| 覆盖度 | expected 行操作真正产出比例（扣异表多余） | 0.6667 |
| 精准程度 | 被定位行字段值完全正确比例 | 0.0000 |
| 严格通过率 | 整链 100%命中且无多余写入 | 0.0000 |
| 响应ok率 | Agent 自报告执行成功 | 0.6667 |
| 平均多余写入 | 未被 expected 认领的行改动 | 1.6667 |
| 平均异表写入 | 写到 expected 之外的表 | 0.0000 |
| 平均耗时(ms) | 单链端到端 | 48875.0 |
| P50/P95(ms) | | 41365.7 / 77406.0 |
| 总耗时(s) | | 146.6 |
...
| table_sheet_miss | 2 | 1,3 | 路由或 sheet 别名缺失 → 补 table_context/sheet_aliases skill |
| row_missing | 0 | - | add 未落行/modify 未定位行 → 查列定位与主键自增逻辑 |
| field_error | 4 | 1,2,3 | 字段值写错/枚举未解析/类型不符 → 补 column_aliases/enum_mappings |
...
- 定位 1.00 | 覆盖 1.00 | 精准 0.00 | 引用一致 0.00 (0/1) | producer产出 1/2
- 多余写入 4 (异表 0) | 耗时 77406ms

| # | table.sheet | op | produces | 状态 | 字段分 | matched_op |
|---|---|---|---|---|---|---|
| 1 | item/item.xlsx.ItemBase | add | new_item_id | 🟠 located_only | 0.00 | 有 |
...
| 2 | False | 0.00 | 1.00 | 0.00 | 新增一把武器叫'霜寒剑'，item_id 7001，品质4，道具类型为武 |

## 五、内环优化建议

- 表/sheet 路由失误 2 处：补 table_context.yaml 关键词与 sheet_aliases.yaml，覆盖 NPC/对话/刷新等跨表场景。
