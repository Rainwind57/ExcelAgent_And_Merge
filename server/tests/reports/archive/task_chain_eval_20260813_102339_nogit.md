...
| 引用一致率 | 占位符引用闭环成立的比例（task_chain 核心） | 0.6875 |
| producer产出率 | produces 标注的步实际回传新 ID 的比例 | 0.9000 |
| 定位功能 | 命中正确 table+sheet+操作类型 | 0.9091 |
| 覆盖度 | expected 行操作真正产出比例（扣异表多余） | 0.7273 |
| 精准程度 | 被定位行字段值完全正确比例 | 0.3998 |
| 严格通过率 | 整链 100%命中且无多余写入 | 0.0000 |
| 响应ok率 | Agent 自报告执行成功 | 1.0000 |
| 平均多余写入 | 未被 expected 认领的行改动 | 5.0000 |
| 平均异表写入 | 写到 expected 之外的表 | 1.0000 |
| 平均耗时(ms) | 单链端到端 | 143795.4 |
| P50/P95(ms) | | 143795.4 / 143795.4 |
| 总耗时(s) | | 143.8 |
...
| table_sheet_miss | 1 | 1 | 路由或 sheet 别名缺失 → 补 table_context/sheet_aliases skill |
| row_missing | 1 | 1 | add 未落行/modify 未定位行 → 查列定位与主键自增逻辑 |
| field_error | 7 | 1 | 字段值写错/枚举未解析/类型不符 → 补 column_aliases/enum_mappings |
| ref_broken | 3 | 1 | 占位符替换错误或 consumer 字段名错 → 修 OperationOrchestrator._capture_produced 列名派生 |
| producer_not_resolved | 2 | 1 | producer 新 ID 未回传 result_rows → 修 _append_row 主键回传/produces 标注 |
...
- 定位 0.91 | 覆盖 0.73 | 精准 0.40 | 引用一致 0.69 (11/16) | producer产出 9/10
- 多余写入 5 (异表 1) | 耗时 143795ms
...
|---|---|---|---|---|---|
| 1 | False | 0.69 | 0.73 | 0.40 | 新增一个任务NPC叫'寻宝老人'，model_id 1019，放在spa |
...
- 字段错误 7 处：补 column_aliases / enum_mappings / value_constraints，强化枚举值预解析与类型校验。
- 引用断裂 3 处 + producer 未产出 2 处：这是 task_chain 核心瓶颈。核查 OperationOrchestrator._capture_produced 主键列名派生（首列 col==1 优先）与 _resolve_placeholders 占位符替换覆盖；确保 add 结果 result_rows 回传主键新值，produces 标签与占位符名对齐。
