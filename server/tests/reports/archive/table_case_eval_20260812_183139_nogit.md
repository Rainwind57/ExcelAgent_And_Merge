...
| 平均耗时(ms) | 单条指令端到端耗时（含二次确认续传） | 195418.6 | 502720.4 | -307301.8 |
| P50耗时(ms) | | 195418.6 | 502720.4 | |
| P95耗时(ms) | | 195418.6 | 502720.4 | |
| 需二次确认比例 | 触发级联删除等确认流程的比例 | 0.0000 | 0.0000 | |
| 总耗时(s) | 本轮全部样例累计耗时 | 195.4 | 502.7 | |
...
- skill 挂载后耗时变化: -307301.8ms（变慢，正数=on比off快，因为定义为 off-on）
- 综合判定: skill 增强 效果不明显
