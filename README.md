...
**访问：**
- `http://127.0.0.1:8000` — 生产模式（后端直出前端页面，本机访问）
- `http://<本机局域网IP>:8000` — 局域网内其他设备通过 IP 访问（后端默认监听 `0.0.0.0`，启动日志会打印本机可用的 IP 访问地址）
- `http://localhost:5173` — 开发模式（`cd frontend && npm install && npm run dev`）
- `http://127.0.0.1:8000/docs` — Swagger API 文档

> 仅需本机访问时，在 `.env` 中设置 `BACKEND_HOST=127.0.0.1` 后重启即可；需要局域网 IP 访问则保持 `BACKEND_HOST=0.0.0.0`（默认）。
> 局域网访问需放行防火墙端口 8000（首次启动 Windows 防火墙弹窗点「允许访问」，或手动添加入站规则）。
